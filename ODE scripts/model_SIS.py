...
class SIS_Model:
    def __init__(self,beta,gamma,mu,nu,rho,tau,theta):
    ...
    def compute(self,maxtick,S0,I0,F0):
        ...
        for t in range(maxtick-1):
            ...
            F_k = F
            try:
                S = S_k + self.mu * N_k - self.beta * S_k * I_k / N_k + self.gamma * I_k - self.nu * S_k - self.rho * S_k / F_k
    ...
    def compute_rhs(self, t, v):
        S,I,F = v
        N = S + I
        dS = self.mu * N - self.beta * S * I / N + self.gamma * I - self.nu * S - self.rho * S / F
...
if __name__ == '__main__':
    ...
    example = SIS_Model(beta=0.67,gamma=0.12,mu=0.0,nu=0.0,rho=0.0,tau=0.0,theta=0.0)
    example.plot(maxtick=50,S0=450,I0=450,F0=50000)
