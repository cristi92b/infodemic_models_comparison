...
class SEIRS_Model:
    def __init__(self,beta,gamma,sigma,xi,mu,nu,rho,tau,theta):
    ...
    def compute(self,maxtick,S0,E0,I0,R0,F0):
        ...
        for t in range(maxtick-1):
            ...
            try:
                ...
                F = F_k  - self.tau * F_k * N_k / P  + 100 * self.theta * N_k / F_k #self.tau * N_k / F_k + self.theta * P * N_k / F_k # F = 50000 - 0.1 * 150 + 0.03 * 1089
                if F > 108900:
                    F = 108900
                N = S + I + R + E
                S_array.append(S)
    ...
    def compute_rhs(self, t, v):
        ...
        dF = - self.tau * F * N / self.P  + 100 * self.theta * N / F
...
if __name__ == '__main__':
    ...
    example = SEIRS_Model(beta=0.5,gamma=1.0,sigma=0.1,xi=0.1,mu=0.0,nu=0.0,rho=0.0,tau=0.0,theta=0.0)
